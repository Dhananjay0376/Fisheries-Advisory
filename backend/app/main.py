...
from app.models import Advisory, Subscriber, Region, User
...
def seed_dummy_data(db: Session = Depends(get_db)):
    """Convenience endpoint to seed the database with admin user, regions, and dummy advisories."""
    # 1. Seed Admin User
    admin_seeded = False
    ...
    if db.query(Region).count() == 0:
        default_regions = [
            ...
            Region(name="vizag", latitude=17.6868, longitude=83.2185),
            Region(name="kochi", latitude=9.9312, longitude=76.2673),
            Region(name="mumbai", latitude=19.0760, longitude=72.8777),
            Region(name="mangalore", latitude=12.9141, longitude=74.8560)
        ]
        db.add_all(default_regions)
        regions_seeded = len(default_regions)
    ...
    if db.query(Advisory).count() == 0:
        dummy_advisories = [
            ...
            Advisory(
                ...
                content_ta="சென்னை கடற்கரையில் இருந்து 25 கிமீ தொலைவில் அதிக குளோரோபில் செறிவு மற்றும் தகவல் சாதகமான கடல் வெப்பநிலை கண்டறியப்பட்டுள்ளது. நல்ல சூரை மீன் பிடிப்பு எதிர்பார்க்கப்படுகிறது.",
            ...
            Advisory(
                ...
                content_hi="केरल तट पर 2.5 - 3.1 मीटर ऊंची लहरों की भविष्यवाणी। पुलिस ने सावधानी बरतने का अनुरोध किया है।",
                latitude=10.0,
                longitude=76.0,
                radius_km=150.0,
        ...
        advisories_seeded = len(dummy_advisories)
    
    db.commit()
    return {
        "message": "Database seeded successfully",
        "admin_created": admin_seeded,
        "regions_seeded": regions_seeded,
        "subscribers_seeded": subs_seeded,
        "advisories_seeded": advisories_seeded
    }
